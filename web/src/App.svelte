<script lang="ts">
  ...
  import bwomp from "./assets/bwomp.mp3";
  ...
  function collectObstacles() {
    ...
    els.forEach((el) => {
      if (el.dataset.piece) return // skip pieces
  ...
  function measurePieces() {
    ...
    pieces.forEach((p) => {
      ...
      if (p.x === 0 && p.y === 0) {
        ...
        const speed = 20 + Math.random() * 40 // px/s, gentle
  ...
  let route = $state<'/' | '/create' | '/join'>('/');

  function parseRoute() {
    const h = location.hash.replace(/^#/, '') || '/';
    if (h.startsWith('/create')) route = '/create';
    else if (h.startsWith('/join')) route = '/join';
    else route = '/';
  }
  ...
  // shared bold, edgy CTA button style
  const btnClass = "group relative inline-flex items-center justify-center cursor-pointer select-none bg-[#E1FF00] text-black font-poppins font-semibold uppercase tracking-wide text-[42px] md:text-[48px] px-[72px] md:px-[100px] py-[18px] md:py-[20px] rounded-[16px] transition-all duration-200 ease-out hover:rounded-[999px] hover:-rotate-1 hover:scale-[1.02] active:scale-[0.98] shadow-[0_10px_0_#a4bf00,0_0_0_2px_#000_inset] hover:shadow-[0_12px_0_#a4bf00,0_0_0_2px_#000_inset] focus:outline-none focus:ring-4 focus:ring-[#E1FF00]/30";

  const clickSound = new Audio(bwomp);
...
<div bind:this={containerEl} class="relative bg-[#090909] min-h-screen w-full overflow-hidden select-none">
  ...
  {:else}
      <img src="/cactus.svg" alt="Cactus" class="top-0 right-0 absolute w-[clamp(120px,20vw,320px)]" data-obstacle />
      <img src="/monster_eyes.svg" alt="Monster Eyes" class="-bottom-[5vh] right-[10vw] absolute w-[clamp(300px,30vw,400px)]" data-obstacle />

      {#if route === '/'}
        <div class="flex flex-col bottom-[100px] left-[100px] absolute gap-[12px]" data-obstacle>
            <button type="button" class={btnClass} onclick={() => { playClickSound(); nav('/create'); }}>
                <span class="flex items-center gap-4">
                  <span>create game</span>
                  <span aria-hidden="true" class="transition-transform duration-200 ease-out translate-x-0 group-hover:translate-x-1">➜</span>
                </span>
            </button>
            <button type="button" class={btnClass} onclick={() => { playClickSound(); nav('/join'); }}>
                <span class="flex items-center gap-4">
                  <span>join game</span>
                  <span aria-hidden="true" class="transition-transform duration-200 ease-out translate-x-0 group-hover:translate-x-1">➜</span>
                </span>
      ...
      {:else if route === '/create'}
        <div class="absolute inset-0 overflow-auto" data-obstacle>
          <CreateGame />
        </div>
      {:else if route === '/join'}
        <div class="absolute inset-0 overflow-auto" data-obstacle>
